...
CAPABILITIES: tuple[CapabilityDefinition, ...] = (
    ...
    CapabilityDefinition(
        intent="get_encounters",
        handler_name="_handle_get_encounters",
        summary="List encounters/visits for a patient.",
        fields=(CapabilityField("patient_query", "Patient name or identifier.", required=False),),
        examples=("Show encounters for Maria", "List visits for Nesh Rochwani", "What encounters does this patient have?"),
    ),
    CapabilityDefinition(
        intent="get_visits",
        handler_name="_handle_get_visits",
        summary="List patient visits from the OpenMRS visit module.",
        fields=(CapabilityField("patient_query", "Patient name or identifier.", required=False),),
        examples=("Show visits for Maria", "What visits has this patient had?"),
    ),
    CapabilityDefinition(
        intent="create_clinical_note",
        handler_name="_handle_create_clinical_note",
        summary="Record a free-text clinical note, progress note, or chief complaint for a patient.",
        fields=(
            CapabilityField("patient_query", "Patient name or identifier.", required=False),
            CapabilityField("note_text", "The clinical note content.", required=True),
            CapabilityField("note_type", "Type: note, chief complaint, clinical impression, assessment.", required=False),
        ),
        write=True,
        examples=("Add note for Maria: Patient presents with chest pain", "Record chief complaint: shortness of breath"),
    ),
    CapabilityDefinition(
        intent="search_drugs",
        handler_name="_handle_search_drugs",
        summary="Search available drugs/medications in OpenMRS formulary.",
        scope="global",
        fields=(CapabilityField("drug_query", "Drug name to search.", required=True),),
        examples=("Search for metformin", "What drugs contain paracetamol?", "Is amoxicillin available?"),
    ),
    CapabilityDefinition(
        intent="search_providers",
        handler_name="_handle_search_providers",
        summary="Search OpenMRS providers/clinicians.",
        scope="global",
        fields=(CapabilityField("provider_query", "Provider name or identifier.", required=False),),
        examples=("Show all providers", "Search for provider John", "List available clinicians"),
    ),
    CapabilityDefinition(
        intent="search_locations",
        handler_name="_handle_search_locations",
        summary="Search OpenMRS locations/wards/clinics.",
        scope="global",
        fields=(CapabilityField("location_query", "Location name to search.", required=False),),
        examples=("Show all locations", "What wards are available?", "List outpatient clinics"),
    ),
    CapabilityDefinition(
        intent="get_programs",
        handler_name="_handle_get_programs",
        summary="List patient program enrollments or all available programs.",
        fields=(CapabilityField("patient_query", "Patient name or identifier.", required=False),),
        examples=("What programs is Maria enrolled in?", "List all programs", "Show patient programs"),
    ),
    CapabilityDefinition(
        intent="get_encounter_types",
        handler_name="_handle_get_encounter_types",
        summary="List all available encounter types in OpenMRS.",
        scope="global",
        examples=("What encounter types are available?", "List encounter types"),
    ),
)
