...
from app.models.common import ChatResponseEnvelope, EntityResult, PendingActionRecord, WorkflowStep
...
class ChatAgentService:
    ...
    def handle_message(self, prompt: str, actor: Actor, *, patient_uuid: str | None = None, attachment_path: str | None = None) -> ChatResponseEnvelope:
        deterministic = self.parser.parse(prompt, has_file=bool(attachment_path))
        parsed = self.reasoning.resolve_intent(prompt, deterministic, has_file=bool(attachment_path))
        ...
        response = handler(prompt, parsed.entities, actor, workflow, patient_uuid=patient_uuid, attachment_path=attachment_path)
        self._log(actor, parsed.intent, response.patient_context.get("uuid") if response.patient_context else patient_uuid, prompt, "preview" if response.pending_action else "completed")
        return response
    ...
    def _resolve_patient_context(self, entities: dict[str, Any], explicit_patient_uuid: str | None, workflow: list[WorkflowStep]) -> dict[str, Any]:
        patient = self.patients.resolve_patient(entities.get("patient_query"), explicit_patient_uuid)
    ...
    def _handle_search_patient(self, prompt: str, entities: dict[str, Any], actor: Actor, workflow: list[WorkflowStep], **_: Any) -> ChatResponseEnvelope:
        ...
        results = self.patients.search(query)
        ...
        return ChatResponseEnvelope(
            ...
            message=f"I found {len(results)} patient candidate(s) for '{query}'.",
            workflow=workflow,
            patient_context=context,
            data=results,
        )

    def _handle_patient_analysis(self, prompt: str, entities: dict[str, Any], actor: Actor, workflow: list[WorkflowStep], *, patient_uuid: str | None = None, **_: Any) -> ChatResponseEnvelope:
        ensure_permission(actor, "read:clinical")
        context = self._resolve_patient_context(entities, patient_uuid, workflow)
        ...
        llm_summary = self.reasoning.render_clinical_summary(context["display"], brief)
        return ChatResponseEnvelope(
    ...
    def _handle_get_observations(self, prompt: str, entities: dict[str, Any], actor: Actor, workflow: list[WorkflowStep], *, patient_uuid: str | None = None, **_: Any) -> ChatResponseEnvelope:
        ensure_permission(actor, "read:clinical")
        context = self._resolve_patient_context(entities, patient_uuid, workflow)
    ...
    def _handle_get_conditions(self, prompt: str, entities: dict[str, Any], actor: Actor, workflow: list[WorkflowStep], *, patient_uuid: str | None = None, **_: Any) -> ChatResponseEnvelope:
        ensure_permission(actor, "read:clinical")
        context = self._resolve_patient_context(entities, patient_uuid, workflow)
    ...
    def _handle_get_allergies(self, prompt: str, entities: dict[str, Any], actor: Actor, workflow: list[WorkflowStep], *, patient_uuid: str | None = None, **_: Any) -> ChatResponseEnvelope:
        ensure_permission(actor, "read:clinical")
        context = self._resolve_patient_context(entities, patient_uuid, workflow)
    ...
    def _handle_get_medications(self, prompt: str, entities: dict[str, Any], actor: Actor, workflow: list[WorkflowStep], *, patient_uuid: str | None = None, **_: Any) -> ChatResponseEnvelope:
        ensure_permission(actor, "read:clinical")
        context = self._resolve_patient_context(entities, patient_uuid, workflow)
    ...
    def _handle_get_medication_dispense(self, prompt: str, entities: dict[str, Any], actor: Actor, workflow: list[WorkflowStep], *, patient_uuid: str | None = None, **_: Any) -> ChatResponseEnvelope:
        ensure_permission(actor, "read:clinical")
        context = self._resolve_patient_context(entities, patient_uuid, workflow)
    ...
    def _handle_create_patient(self, prompt: str, entities: dict[str, Any], actor: Actor, workflow: list[WorkflowStep], **_: Any) -> ChatResponseEnvelope:
        ...
        if not all([entities.get("given_name"), entities.get("family_name"), entities.get("birthdate")]):
            raise ValidationError("Patient creation requires first name, last name, gender, and birthdate in the prompt.")
    ...
    def _handle_create_encounter(self, prompt: str, entities: dict[str, Any], actor: Actor, workflow: list[WorkflowStep], *, patient_uuid: str | None = None, **_: Any) -> ChatResponseEnvelope:
    ...
    def _handle_create_observation(self, prompt: str, entities: dict[str, Any], actor: Actor, workflow: list[WorkflowStep], *, patient_uuid: str | None = None, **_: Any) -> ChatResponseEnvelope:
    ...
    def _handle_update_observation(self, prompt: str, entities: dict[str, Any], actor: Actor, workflow: list[WorkflowStep], *, patient_uuid: str | None = None, **_: Any) -> ChatResponseEnvelope:
        ensure_permission(actor, "write:observation")
        context = self._resolve_patient_context(entities, patient_uuid, workflow)
        observations = entities.get("observations", [])
    ...
    def _handle_delete_observation(self, prompt: str, entities: dict[str, Any], actor: Actor, workflow: list[WorkflowStep], *, patient_uuid: str | None = None, **_: Any) -> ChatResponseEnvelope:
    ...
    def _handle_create_condition(self, prompt: str, entities: dict[str, Any], actor: Actor, workflow: list[WorkflowStep], *, patient_uuid: str | None = None, **_: Any) -> ChatResponseEnvelope:
    ...
    def _handle_update_condition(self, prompt: str, entities: dict[str, Any], actor: Actor, workflow: list[WorkflowStep], *, patient_uuid: str | None = None, **_: Any) -> ChatResponseEnvelope:
        ensure_permission(actor, "write:condition")
        context = self._resolve_patient_context(entities, patient_uuid, workflow)
    ...
    def _handle_delete_condition(self, prompt: str, entities: dict[str, Any], actor: Actor, workflow: list[WorkflowStep], *, patient_uuid: str | None = None, **_: Any) -> ChatResponseEnvelope:
    ...
    def _handle_create_allergy(self, prompt: str, entities: dict[str, Any], actor: Actor, workflow: list[WorkflowStep], *, patient_uuid: str | None = None, **_: Any) -> ChatResponseEnvelope:
    ...
    def _handle_update_allergy(self, prompt: str, entities: dict[str, Any], actor: Actor, workflow: list[WorkflowStep], *, patient_uuid: str | None = None, **_: Any) -> ChatResponseEnvelope:
        ensure_permission(actor, "write:allergy")
        context = self._resolve_patient_context(entities, patient_uuid, workflow)
    ...
    def _handle_delete_allergy(self, prompt: str, entities: dict[str, Any], actor: Actor, workflow: list[WorkflowStep], *, patient_uuid: str | None = None, **_: Any) -> ChatResponseEnvelope:
    ...
    def _handle_create_medication(self, prompt: str, entities: dict[str, Any], actor: Actor, workflow: list[WorkflowStep], *, patient_uuid: str | None = None, **_: Any) -> ChatResponseEnvelope:
    ...
    def _handle_create_medication_dispense(self, prompt: str, entities: dict[str, Any], actor: Actor, workflow: list[WorkflowStep], *, patient_uuid: str | None = None, **_: Any) -> ChatResponseEnvelope:
        ensure_permission(actor, "write:medication")
        context = self._resolve_patient_context(entities, patient_uuid, workflow)
    ...
    def _handle_update_medication(self, prompt: str, entities: dict[str, Any], actor: Actor, workflow: list[WorkflowStep], *, patient_uuid: str | None = None, **_: Any) -> ChatResponseEnvelope:
        ensure_permission(actor, "write:medication")
        context = self._resolve_patient_context(entities, patient_uuid, workflow)
    ...
    def _handle_ingest_pdf(self, prompt: str, entities: dict[str, Any], actor: Actor, workflow: list[WorkflowStep], *, patient_uuid: str | None = None, attachment_path: str | None = None, **_: Any) -> ChatResponseEnvelope:
    ...
    def _handle_sync_health_gorilla(self, prompt: str, entities: dict[str, Any], actor: Actor, workflow: list[WorkflowStep], **_: Any) -> ChatResponseEnvelope:
        ensure_permission(actor, "write:ingestion")
        if not all([entities.get("given_name"), entities.get("family_name"), entities.get("birthdate")]):
