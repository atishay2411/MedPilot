...
# PDF parsing service — ported from lab parsing.py
import pdfplumber
import re
import json
...
def parse_pdf(path: str) -> dict:
    """
    Parse a structured patient record PDF and return structured data dict.
    Supports: Demographics, Problem List, Vital Signs, Allergies, Medications
    """
    parsed = {
    ...
    while i < len(lines):
        ...
        if current_section == "Vital Signs":
            if re.match(r"Weight.*\d", line):
                ...
                if "/" in bp_val:
                    try:
                        sys_bp, dia_bp = bp_val.split("/")
                        parsed["observations"]["Systolic blood pressure"] = int(sys_bp.strip())
                        parsed["observations"]["Diastolic blood pressure"] = int(dia_bp.strip())
                    except Exception:
            ...
            if line.startswith("Drug:"):
                ...
                if ":" in line:
                    ...
                    if key == "Concept":
                        ...
                        if num is not None:
                            current_med["dose"] = num
                        if unit:
                            current_med["dose_units_name"] = unit
                    elif key == "Route":
                        ...
                        num, unit = parse_numeric_and_unit(val)
                        if num is not None:
                            current_med["duration"] = num
                        if unit:
                            current_med["duration_units_name"] = unit
                    elif key == "Quantity":
                        num, unit = parse_numeric_and_unit(val)
                        if num is not None:
                            current_med["quantity"] = num
                        if unit:
                            current_med["quantity_units_name"] = unit
                    elif key == "Refills":
                        try:
